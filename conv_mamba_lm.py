...
from conv_mamba import Mamba, MambaConfig, RMSNorm
...
class MambaLM(nn.Module):
    def __init__(self, lm_config: MambaLMConfig):
        ...
        self.timesteps = lm_config.timesteps

        # self.embedding = nn.Embedding(self.lm_config.vocab_size, self.config.d_model)
        self.mamba = Mamba(self.config)
        self.norm_f = RMSNorm(self.config.d_model)

        self.in_head = nn.Conv3d(lm_config.in_dim, self.config.d_model, kernel_size=[self.timesteps, 1, 1], padding=[self.timesteps - 1, 1//2, 1//2], bias=True)
        # self.out_head = nn.Conv3d(self.config.d_model, lm_config.out_dim, kernel_size=[self.timesteps, 1, 1], padding=[1//2, 1//2, 1//2], bias=True)
        self.out_head = nn.Linear(self.config.d_model, lm_config.out_dim)
    ...
    def forward(self, tokens):
        ...
        x = self.in_head(x)
        x = self.mamba(x)
        x = self.norm_f(x)  # Only needed when mapping to/from a tokenizer

        # GAP
        x = x.reshape(x.shape[0], x.shape[1], -1).max(-1)[0]
    ...
    def step(self, token, caches):
        ...
        # logits = self.lm_head(x)  # Only needed when mapping to/from a tokenizer
